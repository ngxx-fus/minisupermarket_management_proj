...
void ManagerProduct::on_pushButton_clicked()
{
    ...
    if (j==-1)
    {
        ...
        int h= CompareDate(day,month, year, dayexp, monthexp, yearexp);
        int k = DateAnalysis(day, month, year, dayexp,  monthexp,  yearexp );
        if (k==0){
            if(h==1){
                if(i == -1){
                ...
                ui->txtAmount->clear();
                ui->txtPrice->clear();
                QMessageBox::information(this, "ID in Commodities", "Hạn sử dụng sai ");
            ...
            //            ui->txtAmount->clear();
            //            ui->txtPrice->clear();
            QMessageBox::information(this, "ID in Commodities", "Nhập ngày sai ");
